...
public class DataController : Controller
{
    ...
    private async Task<bool> RunPythonScript(int startYear)
    {
        try
        {
            _logger.LogInformation($"Attempting to run Python script for NBA data update from year {startYear}");
            
            var pythonScriptPath = Path.Combine(Directory.GetCurrentDirectory(), "update_data.py");
            if (!System.IO.File.Exists(pythonScriptPath))
            {
                _logger.LogWarning($"Python script not found at: {pythonScriptPath}");
                return false;
            }
            ...
            {
                FileName = "python3",
                Arguments = $"\"{pythonScriptPath}\" --start-year {startYear}",
            ...
            process.OutputDataReceived += (sender, e) => 
            {
                if (!string.IsNullOrEmpty(e.Data))
                {
                    ...
                    _logger.LogInformation($"Python script output: {e.Data}");
                }
            };
            ...
            process.ErrorDataReceived += (sender, e) => 
            {
                if (!string.IsNullOrEmpty(e.Data))
                {
                    ...
                    _logger.LogWarning($"Python script error: {e.Data}");
                }
            };
    ...
    private async Task<bool> ImportFromExistingCsv()
    {
        try
        {
            var csvFiles = new[]
            {
                "nba_player_stats_2023_present.csv",
                "../nba_player_stats_2023_present.csv",
                "database_24_25.csv"
            };
            ...
            {
                var csvPath = Path.Combine(Directory.GetCurrentDirectory(), csvFile);
                if (System.IO.File.Exists(csvPath))
                {
                    _logger.LogInformation($"Found CSV file at: {csvPath}");
                    ...
                    {
                        var gameStatDict = (IDictionary<string, object>)record;
                        var gameStat = ConvertCsvRecordToGameStats(gameStatDict);
                        if (gameStat != null)
                        {
                            gameStats.Add(gameStat);
                        }
                    ...
                    if (gameStats.Any())
                    {
                        await _context.GameStats.AddRangeAsync(gameStats);
                        await _context.SaveChangesAsync();
                        _logger.LogInformation($"Imported {gameStats.Count} records from CSV file");
                        return true;
                    }
                }
            }
            
            _logger.LogWarning("No suitable CSV files found for import");
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error importing from CSV file");
            return false;
        }
    }

    private GameStats? ConvertCsvRecordToGameStats(IDictionary<string, object> record)
    {
        try
        {
            var gameStat = new GameStats
            {
                Player = GetCsvValue(record, "Player") ?? "",
                Team = GetCsvValue(record, "Tm") ?? "",
                Opponent = GetCsvValue(record, "Opp") ?? "",
                Result = GetCsvValue(record, "Res") ?? ""
            };

            if (double.TryParse(GetCsvValue(record, "MP"), out double mp))
                gameStat.MinutesPlayed = mp;
            
            if (int.TryParse(GetCsvValue(record, "FG"), out int fg))
                gameStat.FieldGoalsMade = fg;
                
            if (int.TryParse(GetCsvValue(record, "FGA"), out int fga))
                gameStat.FieldGoalsAttempted = fga;
                
            if (double.TryParse(GetCsvValue(record, "FG%"), out double fgPct))
                gameStat.FieldGoalPercentage = fgPct;
                
            if (int.TryParse(GetCsvValue(record, "3P"), out int threeP))
                gameStat.ThreePointersMade = threeP;
                
            if (int.TryParse(GetCsvValue(record, "3PA"), out int threePA))
                gameStat.ThreePointersAttempted = threePA;
                
            if (double.TryParse(GetCsvValue(record, "3P%"), out double threePct))
                gameStat.ThreePointPercentage = threePct;
                
            if (int.TryParse(GetCsvValue(record, "FT"), out int ft))
                gameStat.FreeThrowsMade = ft;
                
            if (int.TryParse(GetCsvValue(record, "FTA"), out int fta))
                gameStat.FreeThrowsAttempted = fta;
                
            if (double.TryParse(GetCsvValue(record, "FT%"), out double ftPct))
                gameStat.FreeThrowPercentage = ftPct;
                
            if (int.TryParse(GetCsvValue(record, "ORB"), out int orb))
                gameStat.OffensiveRebounds = orb;
                
            if (int.TryParse(GetCsvValue(record, "DRB"), out int drb))
                gameStat.DefensiveRebounds = drb;
                
            if (int.TryParse(GetCsvValue(record, "TRB"), out int trb))
                gameStat.TotalRebounds = trb;
                
            if (int.TryParse(GetCsvValue(record, "AST"), out int ast))
                gameStat.Assists = ast;
                
            if (int.TryParse(GetCsvValue(record, "STL"), out int stl))
                gameStat.Steals = stl;
                
            if (int.TryParse(GetCsvValue(record, "BLK"), out int blk))
                gameStat.Blocks = blk;
                
            if (int.TryParse(GetCsvValue(record, "TOV"), out int tov))
                gameStat.Turnovers = tov;
                
            if (int.TryParse(GetCsvValue(record, "PF"), out int pf))
                gameStat.PersonalFouls = pf;
                
            if (int.TryParse(GetCsvValue(record, "PTS"), out int pts))
                gameStat.Points = pts;
                
            if (double.TryParse(GetCsvValue(record, "GmSc"), out double gmSc))
                gameStat.GameScore = gmSc;

            var dateStr = GetCsvValue(record, "Data") ?? GetCsvValue(record, "Date") ?? GetCsvValue(record, "GAME_DATE");
            if (!string.IsNullOrEmpty(dateStr))
            {
                ...
                else if (DateTime.TryParseExact(dateStr, "yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture, DateTimeStyles.None, out gameDate))
                {
                    gameStat.GameDate = gameDate.Date;
                }
                else if (DateTime.TryParseExact(dateStr, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out gameDate))
                {
                    gameStat.GameDate = gameDate.Date;
                }
                else
        ...
        {
            _logger.LogError(ex, "Error converting CSV record to GameStats");
            return null;
        }
    }

    private string? GetCsvValue(IDictionary<string, object> record, string key)
    {
